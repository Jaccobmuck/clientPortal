...
export function renderInvoiceSent(vm: InvoiceEmailViewModel): RenderedEmail {
  const subject = `Invoice ${vm.invoiceNumber} from ${vm.orgName}`;

  const dueLine = vm.dueDateFormatted
    ? `<p style="margin:0 0 8px;color:#555;">Due: <strong>${vm.dueDateFormatted}</strong></p>`
    : "";

  const dueDateText = vm.dueDateFormatted ? `Due: ${vm.dueDateFormatted}\n` : "";
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#1a1a1a;max-width:600px;margin:0 auto;padding:20px;">
  <h2 style="margin:0 0 24px;color:#1a1a1a;">New Invoice from ${vm.orgName}</h2>
  <p style="margin:0 0 8px;">Hi ${vm.clientName},</p>
  <p style="margin:0 0 16px;">You have a new invoice for <strong>${vm.totalFormatted}</strong>.</p>
  ...
  const text = `New Invoice from ${vm.orgName}

Hi ${vm.clientName},

You have a new invoice for ${vm.totalFormatted}.

Invoice: ${vm.invoiceNumber}
Amount: ${vm.totalFormatted}
${dueDateText}${payLinkText}
${vm.orgName}`;
